...
// ─────────────────────────────────────────────────────────────
// featureCounts adapter
// ─────────────────────────────────────────────────────────────

#[test]
fn test_featurecounts_adapter_strips_annotation_columns() {
    let tmp = TempDir::new().unwrap();
    let tsv = tmp.path().join("counts.featureCounts.tsv");
    {
        use std::io::Write;
        let mut f = std::fs::File::create(&tsv).unwrap();
        writeln!(f, "# Program:featureCounts v2.0.6, generated by RustQC").unwrap();
        writeln!(f, "Geneid\tChr\tStart\tEnd\tStrand\tLength\tS01\tS02").unwrap();
        writeln!(f, "ENSG00000001\t1\t100\t200\t+\t101\t12\t34").unwrap();
        writeln!(f, "ENSG00000002\t2\t10\t20\t-\t11\t0\t7").unwrap();
    }
    let out = tmp.path().join("counts.parquet");
    rustpipe(&[
        "convert",
        "--from",
        "featurecounts",
    ...
    assert!(out.exists(), "parquet was not created");
    let cols = parquet_columns(&out);
    assert_eq!(
        cols,
        vec!["Geneid".to_string(), "S01".to_string(), "S02".to_string()],
        "annotation columns must be stripped: {:?}",
        cols
    );
    let genes = read_string_column(&out, "Geneid");
    assert_eq!(genes, vec!["ENSG00000001".to_string(), "ENSG00000002".to_string()]);
    let df = LazyFrame::scan_parquet(&out, Default::default())
        .unwrap()
        .collect()
        .unwrap();
    let s01 = df
    ...
    let s02 = df
        ...
        .map(|v| v.unwrap())
        .collect::<Vec<_>>();
    assert_eq!(s01, vec![12, 0]);
    assert_eq!(s02, vec![34, 7]);
}
